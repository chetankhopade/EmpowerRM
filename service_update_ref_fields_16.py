...
def update_history_chargebacks_disputes(db):
    ...
    chargebacks_history_to_be_processed = ChargeBackHistory.objects.using(db).only('cbid', 'id')
    for cbh in chargebacks_history_to_be_processed:
        cb_disputes = ChargeBackDispute.objects.using(db).filter(Q(chargeback_id=cbh.id) | Q(chargeback_ref_id=cbh.id)).distinct()
        if cb_disputes.exists():
            move_chargebackdisputes_to_chargebackdispute_history_table(cb_disputes, db)
            print(f"Moved CBDisputes to CBHistoryDisputes for CB: {cbh.cbid})")

    chargebackslines_history_to_be_processed = ChargeBackLineHistory.objects.using(db).only('cblnid', 'id')
    for cblnh in chargebackslines_history_to_be_processed:
        cb_disputes = ChargeBackDispute.objects.using(db).filter(Q(chargebackline_id=cblnh.id) | Q(chargebackline_ref_id=cblnh.id)).distinct()
        if cb_disputes.exists():
            move_chargebackdisputes_to_chargebackdispute_history_table(cb_disputes, db)
            print(f"Moved CBDisputes to CBHistoryDisputes for CBLN: {cblnh.cblnid})")
